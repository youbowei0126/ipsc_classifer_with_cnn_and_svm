...
def heatmap(a_img, model, layer_name, channel, limit_id, show_color_bar=False):
    ...
    plt.imshow(
        activation_map, cmap="jet", vmin=limit[limit_id][0], vmax=limit[limit_id][1]
    )
    plt.axis("off")
    if show_color_bar:
...
def heatmap4(img_path, model, layer_name, channel, row, total_row):
    a_img = tiff.imread(img_path)
    plt.subplot(total_row, len(layer_name) + 1, (row - 1) * (len(layer_name) + 1) + 1)
    plt.imshow(
        np.dstack(
            (a_img[:, :, 0] * 1.5 / 64876, a_img[:, :, 0] * 0, a_img[:, :, 0] * 0)
        ),
    )
    plt.axis("off")
...
limit = [[0, 64876], [0, 3000], [0, 6000], [-10, 150], [0, 3000]]
img_path_ = [
...
for i in range(totalrow):
    heatmap4(img_path_[i], model, layer_name, channel, i + 1, totalrow)
plt.savefig(r"visualize/heatmap/heatmap.png", transparent=True, dpi=600)
plt.show()
