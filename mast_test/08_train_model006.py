...
def load_dataset_with_augmentation(path):
    def augment_data(data, labels):
        """
        Perform data augmentation by flipping images vertically, horizontally, and both.
        :param data: Original dataset (numpy array).
        :param labels: Corresponding labels.
        :return: Augmented dataset and labels.
        """
        # Vertical flip
        data_vflip = np.flip(data, axis=1)
        labels_vflip = labels

        # Horizontal flip
        data_hflip = np.flip(data, axis=2)
        labels_hflip = labels

        # Both flips
        data_both_flip = np.flip(data_vflip, axis=2)
        labels_both_flip = labels

        # Concatenate original and augmented data
        augmented_data = np.concatenate([data, data_vflip, data_hflip, data_both_flip], axis=0)
        augmented_labels = np.concatenate([labels, labels_vflip, labels_hflip, labels_both_flip], axis=0)

        return augmented_data, augmented_labels

    print('Start loading dataset with augmentation...')
    train_path = path + "/train/"
    test_path = path + "/test/"
    cata = os.listdir(train_path)
    cata_number = len(cata)
    ...
    for i in range(len(cata)):
        ...
        if i == 0:
            ...
            # Initialize datasets
            x_train, y_train = x_train_this_cata, y_train_this_cata
            x_test, y_test = x_test_this_cata, y_test_this_cata
    ...
    return x_train_aug, y_train_aug, x_test_aug, y_test_aug,list_of_cata
...
def main(input_seed):
    ...
    # load dataset
    x_train, y_train, x_test, y_test, list_of_cata = load_dataset_with_augmentation(r"dataset_new\iPSC_Morphologies")
    print(list_of_cata)
    print(x_train.shape)
    print(y_train.shape)
    ...
    if train_new_model:
        ...
        model.save(rf"mast_test\model2\model006_{input_seed}.keras")
        plot_train_history(train_history)
    else:
        model=load_model(rf"mast_test\model2\model006_{input_seed}.keras")
...
# reports=[0]
for i in tqdm(range(2,101,1)):
    # with open(fr"mast_test\log2\{i}.txt", 'w') as file_:
    #     sys.stdout = file_
    #     sys.stdout = sys.__stdout__
    report=main(i)
    joblib.dump(report,rf"mast_test\reports2\report_{i}.joblib")
    plt.close('all') 
    del report
    import gc
    gc.collect()
